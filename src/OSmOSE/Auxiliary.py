...
def haversine(lat1, lat2, lon1, lon2):
    ...
    return 2 * 6371 * np.arcsin(np.sqrt(
        np.sin((np.pi / 180) * (lat2 - lat1) / 2) ** 2 + np.cos((np.pi / 180) * lat1) * np.cos(
            (np.pi / 180) * lat2) * np.sin((np.pi / 180) * (lon2 - lon1) / 2) ** 2))
...
def numerical_gps(df):
    if len(df) == 6:
        return calendar.timegm(
            datetime.datetime(int(df['year']), int(df['month']), int(df['day']), int(df['hour']), int(df['minute']),
                              int(df['second'])).timetuple())
    return calendar.timegm(
        datetime.datetime(int(df['year']), int(df['month']), int(df['day']), int(df['hour'])).timetuple())
...
get_datarmor_time = lambda x: calendar.timegm(time.strptime(x, '%Y-%m-%dT%H:%M:%S.000%z'))
...
class Auxiliary():
    ...
    def wind_fetch(self):
        ...
        u10_col = self.df.columns[
            np.where((np.array('-'.join(elem[-3:] for elem in self.df.columns).split('-')) == 'u10'))[0]]
        ...
        with tqdm(total=50, position=0, leave=True) as pbar:
            while (np.any(total_wind_fetch[shore_distance > 5] < 1600)) and (counter < 50):
                dlon, dlat = get_wind_fetch(shore_distance[(shore_distance > 5) & (total_wind_fetch < 1600)],
                ...
                total_wind_fetch[(shore_distance > 5) & (total_wind_fetch < 1600)] += shore_distance[
    ...
    def bathymetry(self):
        ...
        temp_lat, temp_lon = self.latitude[~np.isnan(self.latitude)].to_numpy().astype('float16'), self.longitude[
            ~np.isnan(self.longitude)].to_numpy().astype('float16')
    ...
    def load_era(self, method='other'):
        ...
        if method == 'cube' or method == 'cylinder':
            ...
            dates = pd.Series(temp_stamps['timestamps']).apply(lambda x : datetime.datetime.timestamp(x)).to_numpy()
    ...
    def nearest_era(self, *, time_off=600, lat_off=0.5, lon_off=0.5, variables=['u10', 'v10']):
        ...
        self.stamps[:, :, :, 0] = np.array(list(map(get_era_time, self.stamps[:, :, :, 0].ravel()))).reshape(
            self.stamps[:, :, :, 0].shape)
        ...
        self.df[f'np_{time_off}_{lat_off}_{lon_off}_era'] = np.sqrt(
            self.df[f'np_{time_off}_{lat_off}_{lon_off}_u10'] ** 2 + self.df[
                f'np_{time_off}_{lat_off}_{lon_off}_v10'] ** 2)
        del var
    ...
    def interpolation_era(self):
        ...
        for variable in self.variables:
            ...
            temp_variable = np.full([len(self.timestamps)], np.nan)
            temp_variable[(~np.isnan(self.longitude)) | (~np.isnan(self.latitude))] = j.apply_interp(interp,
                                                                                                     temp_timestamps,
                                                                                                    temp_lat, temp_lon)
    ...
    def cube_era(self, *, time_off=600, lat_off=0.5, lon_off=0.5, variables=['u10', 'v10']):
        ...
        for variable in variables:
            ...
            for i, row in temp_df.iterrows():
                if row.isnull().any():  # Skip iteration if there is a nan value
                    continue
                time_index = np.where((abs(self.stamps['timestamps_epoch'] - row.time) <= time_off))[
                    0]  # Get timestamps satisfyin time offset
                if len(time_index) == 0:  # Skip iteration if no weather source is within time offset for this timestep
                    pbar.update(1)
                    temp_variable[i] = np.nan
                    continue
                lat_index = np.where((abs(self.stamps['latitude'] - row.lat) <= lat_off))[
                    0]  # Get latitude indexes satisfyin lon offset
                if len(lat_index) == 0:  # Skip iteration if no weather source is within lat offset for this timestep
                    pbar.update(1)
                    temp_variable[i] = np.nan
                    continue
                lon_index = np.where((abs(self.stamps['longitude'] - row.lon) <= lon_off))[
                    0]  # Get longitude indexes satisfying lon offset
                ...
                for time_ind in time_index:
                    for lat_ind in lat_index:
                        for lon_ind in lon_index:
                            temp_value.append(
                                var[time_ind, lat_ind, lon_ind])  # Saving values satisfying the conditions
        ...
        self.df[f'cube_{time_off}_{lat_off}_{lon_off}_era'] = np.sqrt(
            self.df[f'cube_{time_off}_{lat_off}_{lon_off}_u10'] ** 2 + self.df[
                f'cube_{time_off}_{lat_off}_{lon_off}_v10'] ** 2)
        del var
    ...
    def cylinder_era(self, *, time_off=600, r='depth', variables=['u10', 'v10']):
        ...
        for variable in variables:
            ...
            for i, row in temp_df.iterrows():
                if row.isnull().any():  # Skip iteration if there is a nan value
                    continue
                time_index = np.where((abs(self.stamps['timestamps_epoch'] - row.time) <= time_off))[
                    0]  # Keep variable values within time offset
                if len(time_index) == 0:
                    pbar.update(1)
                    temp_variable[i] = np.nan
                    continue
                ...
                for k, latitude in enumerate(self.stamps['latitude']):
                    mask[k, np.where((haversine(latitude, row.lat, self.stamps['longitude'], row.lon) <= radius[i]))[
                        0]] = True  # Keep elements within desired distance
                if not mask.any():
                    pbar.update(1)
                    temp_variable[i] = np.nan
                    continue
        ...
        self.df[f'cylinder_{time_off}_{r}_era'] = np.sqrt(
            self.df[f'cylinder_{time_off}_{r}_u10'] ** 2 + self.df[f'cylinder_{time_off}_{r}_v10'] ** 2)
        del var
