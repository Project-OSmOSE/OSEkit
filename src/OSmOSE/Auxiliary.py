        ...
        *,
        ...
        match era:
            case str():
                self.era = (
                    era
                    if os.path.isabs(era)
                    else self.path.joinpath(OSMOSE_PATH.environment, era)
                )
            ...
            case True:
                ...
                assert (
                    len(fns) == 1
                ), "Make sur there is one (and only one) nc file in your era folder"
                self.era = fns[0]
            case _:
                self.era = False
    ...
    def join_gps(self):
        ...
        if isinstance(self.gps_coordinates, pd.DataFrame):
            assert (
                ...
                and ("lat" in self.gps_coordinates.columns)
                and ("lon" in self.gps_coordinates.columns)
