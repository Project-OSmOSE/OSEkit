...
def reshape(chunk_size: int, input_files: str, *, output_dir_path: str = None, verbose : bool = False, overwrite : bool = False) -> List[str]:
    """Reshape all audio files in the folder to be of the specified duration. If chunk_size is superior to the base duration of the files, they will be fused according to their order in the timestamp.csv file in the same folder.
        ...
        -----------            
            ...
            `overwrite`: Deletes the content of `output_dir_path` before writing the results. If it is the same as the `input_files` directory, nothing happens.
        Returns:
        --------
    ...
    if not output_dir_path:
        ...
        if overwrite:
            print("Cannot overwrite input directory! Choose a different output directory instead.")

    if not os.path.exists(output_dir_path):
        os.makedirs(output_dir_path)


    input_timestamp = pd.read_csv(os.path.join(input_dir_path, "timestamp.csv"), header=None, names=["filename", "timestamp"])
    
    if not files: files = list(input_timestamp["filename"])
    ...
    while i < len(files):
        ...
        elif substract_timestamps(input_timestamp, files, i).seconds != len(data):
        ...
        while len(data) > chunk_size * sample_rate:
            data = prevdata if prevdata.size > 1 else data #Quite hacky line, only becomes prevdata if it is at least the second while (since prevdata is reset just before)
        ...
        if (len(prevdata)/sample_rate)/(60) == 0.0:
            i += 1
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-files", "-i", help="The files to be reshaped, as either the path to a directory containing audio files and a timestamp.csv or a list of filenames all in the same directory alongside a timestamp.csv.")
    parser.add_argument("--chunk-size", "-s", type=int, help="The time in seconds of the reshaped files.")
    ...
    parser.add_argument("--verbose", "-v", type=bool, default=True, help="Whether the script prints informative messages. Default is true.")
    ...
    files = reshape(chunk_size=args.chunk_size, input_files=input_files, output_dir_path=args.output_dir, verbose=args.verbose, overwrite=args.overwrite)
