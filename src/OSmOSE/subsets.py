...
class SubsetGenerator:
    ...
    def __post_init__(self):
        ...
        log.info(
            f"Scanning paths and gathering infos from metadata files and {self.path_to_list_of_files.stem}.txt...."
        )

        if isfile(self.path_to_list_of_files):
            self.subset_config, self.all_audio_file = self.scan_directory(
                self.path_to_list_of_files
            )
            ...
            timestamp_metadata_df = pd.read_csv(
                self.subset_config["timestamp_metadata"]
            )
            ...
            if self.remove_ext:
                positive_sample_list_wav = [
                    file[:-4] + ".wav" for file in positive_sample_list
                ]
            else:
                positive_sample_list_wav = [
                    file + ".wav" for file in positive_sample_list
                ]

            # Add n_background negative samples
            negative_samples = [
                f for f in self.all_audio_file if f not in positive_sample_list_wav
            ]
            chosen_background_files = sample(
                population=negative_samples, k=self.n_background_noise
            )
            # Concat both
            self.subset_wav_files = positive_sample_list_wav + chosen_background_files
            # write new timestamps for subset
            timestamp_subset_df = timestamp_metadata_df[
                timestamp_metadata_df.filename.isin(self.subset_wav_files)
            ]
            # write new audio metadata for subset
            audio_subset_metadata_df = audio_metadata_df.copy()
            audio_subset_metadata_df.loc[0, "audio_file_count"] = len(
                timestamp_subset_df
            )
            audio_subset_metadata_df.loc[0, "start_date"] = pd.to_datetime(
                timestamp_subset_df.loc[:, "timestamp"]
            ).min()
            audio_subset_metadata_df.loc[0, "end_date"] = pd.to_datetime(
                timestamp_subset_df.loc[:, "timestamp"]
            ).max()
            # write new spectro_metadata for subset
            # No need, it doesn't change ? double check with ENSTA.
            audio_subset_metadata_df.to_csv(
                Path(self.subset_config["audio_metadata"]).parent
                / "subset_metadata.csv",
                index=False,
            )
            timestamp_subset_df.to_csv(
                Path(self.subset_config["timestamp_metadata"]).parent
                / "subset_timestamp.csv",
                index=False,
            )
            log.info("Writing list of negative files in subset_chosen_background.txt")
            with open(
                Path(self.subset_config["timestamp_metadata"]).parent
                / "subset_chosen_background.txt",
                "w",
            ) as f:
                f.write("\n".join(chosen_background_files))
            log.info(
                f"Number of positive samples selected : {len(positive_sample_list_wav)}"
            )
            log.info(
                f"Number of negative samples selected : {len(chosen_background_files)}"
            )
    ...
    def copy_subset(
        self,
        output_path="",
        replace_original_metadata=True,
        remote_copy=False,
        labels=True,
    ):
        log.info("Creating an empty tree structure...")
        copytree(
            src=self.path_to_list_of_files.parents[4],
            dst=output_path,
        ...
        spectro_path = (
            ...
            / "processed"
            / "spectrogram"
            / f"{self.subset_config['spectro_duration']}_{self.subset_config['sr']}"
            / f"{self.subset_config['nfft']}_{self.subset_config['window_size']}_{self.subset_config['overlap']}_{self.subset_config['custom_frequency_scale']}"
        )
        audio_path = (
            Path(output_path)
            / "data"
            / "audio"
            / f"{str(self.subset_config['spectro_duration'])}_{str(self.subset_config['sr'])}"
        )
    ...
    def scan_directory(input_path):
        ...
        all_audio_files = glob(
            join(dataset_path, "data", "audio", f"{spectro_duration}_{sr}", "*.wav")
        )
