...
def builder_dataset(dataset_ID: str, gps: Union[List[str], str], osmose_group_name:str = "gosmose", force_upload: bool = False) -> Tuple[list, list]:
    """
    ...
    path_dataset = os.path.join(path_osmose_dataset, dataset_ID)

    path_timestamp_formatted = os.path.join(path_dataset,'raw' ,'audio' ,'original','timestamp.csv')
    path_raw_audio = os.path.join(path_dataset,'raw' ,'audio','original')
    
    if not isinstance(gps, list):
        
        csvFileArray = pd.read_csv(os.path.join(path_dataset,'raw' ,'auxiliary' ,gps))
    ...
    if list_filename_abnormal_duration:
        ...
        with open(os.path.join(path_dataset,'raw','audio','files_not_loaded.csv'), 'w') as fp:
    ...
    df.to_csv( os.path.join(path_dataset,'raw' ,'metadata.csv') , index=False)  
    ...
    df.to_csv( os.path.join(path_dataset,'raw' ,'audio','original', 'metadata.csv') , index=False)  
    ...
    for name,duration in zip(list_filename,list_duration):
        if int(duration) < int(nominalVal_duration):
            ...
            list_abnormalFilename_name.append(name)            
            list_abnormalFilename_duration.append(duration)            

        
    
    if ct_abnormal_duration > 0 and not force_upload:
        print('\n \n SORRY but your dataset contains files with different durations, especially',str(len(list_abnormalFilename_name)),'files that have durations smaller than the 10th percentile of all your file durations.. \n')
        ...
        df.to_csv(os.path.join(path_dataset,'raw' ,'audio','original','timestamp.csv'), index=False,na_rep='NaN',header=None)        
        
        # change name of the original wav folder
        new_folder_name = os.path.join(path_dataset,'raw' ,'audio',str(int(pd.DataFrame(list_duration).values.flatten().mean()))+'_'+str(int(float(pd.DataFrame(list_samplingRate).values.flatten().mean()))))
        os.rename( os.path.join(path_dataset,'raw' ,'audio','original') , new_folder_name)
        ...
        if os.path.isfile( os.path.join(path_osmose_dataset , dataset_ID , 'analysis/subset_files.csv') ):
            xx=pd.read_csv(os.path.join(path_osmose_dataset , dataset_ID , 'analysis/subset_files.csv'),header=None).values
            pd.DataFrame([ff[0].replace('-','_') for ff in xx]).to_csv(os.path.join(path_osmose_dataset , dataset_ID , 'analysis/subset_files.csv'),index=False,header=None)   
        ...
        os.chown(path_dataset, -1, gid)
        os.chmod(path_dataset, 0o770)
    ...
    return list_abnormalFilename_name, list_duration


def list_not_built_datasets() -> None:
    """Prints the available datasets that have not been built by the `builder_dataset()` function."""

    dataset_list = [directory for directory in sorted(os.listdir(path_osmose_dataset)) if os.path.isdir(directory) ]

    list_not_built_datasets = []

    for dataset_directory in dataset_list:

        if os.path.exists( os.path.join(path_osmose_dataset,dataset_directory,'raw/audio/original/') ):
...
def delete_abnormal_files(dataset_ID: str,list_filename_abnormal_duration: List[str]) -> None:
    """Delete all files in the given list, and rewrite the timestamps.csv file to reflect the changes.
    
    Parameters:
    -----------
        dataset_ID: the name of the dataset folder
        
        list_filename_abnormal_duration: The list of the abnormal file names to be removed."""
    ...
    for abnormal_file in list_filename_abnormal_duration:
        ...
        print('removing : ',os.path.basename(abnormal_file))
        os.remove(filewav)
