...
class CustomScale(AbstractScale):
    ...
    def __post_init__(self):
        assert all(f >= 0 for f in self.frequencies) and self.sr > 0, "Frequencies should be non-negative"
        assert all(c >= 0 for c in self.coefficients), "Coefficients should be non-negative"
        assert self.frequencies[0] <= self.frequencies[1] <= 0.5*self.sr, "Frequencies for custom scale error - should be sorted f1<f2<0.5*sr"
        assert sum(self.coefficients) == 1 , "Coefficients for custom scale error, - should sum to 1"
    ...
    def map_freq2scale(self, freq):
        ...
        if freq <= f1:
            ...
            return c1 + c2 + (freq - f2) / (0.5*self.sr - f2) * c3 if (f2 != 0.5*self.sr or c3 == 0) else c1 + c2
    ...
    def map_scale2freq(self, custom_freq):
        ...
        if custom_freq <= c1:
            ...
            return f2 + (custom_freq - c1 - c2) / c3 * (0.5*self.sr - f2) if c3 != 0 else 0.5*self.sr