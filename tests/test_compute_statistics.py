...
def test_output_file_written(input_dataset: Path, output_dir: Path):
    ...
    print(output_file)
    compute_stats(input_dir=input_dataset, output_file=output_file, hp_filter_min_freq=100)
...
def test_output_file_columns(input_dataset, output_dir: Path):
    ...
    compute_stats(
        input_dir=str(input_dataset), output_file=str(output_file), hp_filter_min_freq=100
    )
...
def test_output_file_content(input_dataset, output_dir):
    ...
    wav_file1 = input_dataset / "test1.wav"
    data1 = np.random.randn(1000) + 1
    sf.write(wav_file1, data1, 44100)
    wav_file2 = input_dataset / "test2.wav"
    data2 = np.random.randn(1000) * 2
    sf.write(wav_file2, data2, 44100)
    # Call the function to write normalization parameters
    compute_stats(
        input_dir=str(input_dataset), output_file=str(output_file), hp_filter_min_freq=0
    )
    ...
    with open(str(output_file), "r") as f:
        ...
        assert header == ["filename", "mean", "std"]
        row1 = next(reader)
        assert row1[0] == str(input_dataset / "test.wav")
